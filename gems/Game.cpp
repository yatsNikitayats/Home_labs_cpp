...
void Game::render() {
    ...
    for (int i = 0; i < board.getSize(); i++) {
        for (int j = 0; j < board.getSize(); j++) {
            cell.setPosition(sf::Vector2f(
                static_cast<float>(j * 64 + 1),
                static_cast<float>(i * 64 + 1)
            ));

            bool hasBonus = board.hasBonusAt(j, i);
            int bonusType = hasBonus ? static_cast<int>(board.getBonus(j, i).type) : 0;
            cell.setFillColor(getColor(board.getCell(j, i), hasBonus, bonusType));
            window.draw(cell);

            if (hasFirst && fx == j && fy == i && selectionTimer > 0) {
                selectionRect.setPosition(sf::Vector2f(
                    static_cast<float>(j * 64 + 1),
                    static_cast<float>(i * 64 + 1)
                ));
                float alpha = 0.5f + 0.5f * std::sin(selectionTimer * 25.0f);
               
                selectionRect.setOutlineColor(sf::Color(255, 255, 0,
    ...
    for (const auto& eff : dropEffects) {
        ...
        sf::Color col = (eff.type == BonusType::COLORIZE)
            ? sf::Color(0, 255, 255, 180) : sf::Color(50, 50, 50, 180);
